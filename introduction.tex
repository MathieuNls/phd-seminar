...
\begin{itemize}
	\item Work flow integration: Most of these tools are not integrated well with the work flow of software developers. To use these tools, a software developer will need to 
	\item Actionable outcome: The outcome of these tools tend do not lead to actionable items. Most of these tools return several results that are hard to interpret. Take for example, VVV, a bug detection tools...... [COMPLETE THIS]
	\item Leverage of historical data: These tools do not leverage a large body of knowledge that already exist in open source systems. [EXAMPLE]
    \item: End-to-end perspective: The proposed tools work in silos. To complete a task, one needs to download and use multiple tools. For example, ..... [PLEASE COMPLETE THIS]
\end{itemize}

In this thesis, we propose to address the above-mentioned issues by focusing on developing techniques and tools that help software developers at commit-time. [You can add a paragraph here why commit-time should fix the above problems]
